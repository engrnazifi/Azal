...
ADMINS = [6210912739, 5009954635]  # add more admin IDs here
...
ADMIN_DEBUG_ID = os.getenv("ADMIN_DEBUG_ID")
...
DEBUG_GATE = os.getenv("DEBUG_GATE") == "1"
# === PAYMENTS / STORAGE PLACEHOLDERS (fill later) ===
# Set real IDs before deploy:
PAYMENT_NOTIFY_GROUP = -1003553575069  # e.g. -1001234567890 (private group where payment notifications are sent)
...
ADMIN_USERNAME = "Aslamtv1"  # e.g. "Aslamtv2" or None
# ===================== DEBUG GATE SYSTEM =====================
...
def GATE(stage, extra=None, uid=None):
    """
    stage  = mataki (string)
    extra  = karin bayani
    uid    = user id (idan akwai)
    """

    ts = datetime.utcnow().strftime("%H:%M:%S")

    msg = f"🧩 <b>GATE DEBUG</b>\n⏱ {ts}\n➡️ {stage}"
    if uid:
        msg += f"\n👤 UID: <code>{uid}</code>"
    if extra:
        msg += f"\n📝 {extra}"
    ...
    # 1️⃣ print a server
    print(f"[GATE] {stage} | uid={uid} | {extra}")

    # 2️⃣ tura wa admin DM
    if DEBUG_GATE and ADMIN_DEBUG_ID:
        try:
            bot.send_message(
                int(ADMIN_DEBUG_ID),
                msg,
...
def create_flutterwave_payment(user_id, order_id, amount, title):
    ...
    payload = {
        "tx_ref": str(order_id),          # ORDER ID = tx_ref
        "amount": int(amount),
        "currency": "NGN",
...
def flutterwave_callback():
    ...
    <body style="font-family: Arial; text-align: center; padding-top: 50px;">
        ...
        <p>Za a mayar da kai Telegram cikin daƙiƙa 5…</p>
...
def flutterwave_webhook():
    GATE("WEBHOOK:RECEIVED request")

    signature = request.headers.get("verif-hash")
    if not signature:
        GATE("WEBHOOK:FAIL missing verif-hash")
        return "Missing signature", 401

    if signature != FLW_WEBHOOK_SECRET:
        GATE("WEBHOOK:FAIL invalid signature", extra=signature)
        return "Invalid signature", 401

    GATE("WEBHOOK:Signature OK")

    payload = request.json or {}
    GATE("WEBHOOK:Payload received", extra=str(payload)[:500])

    data = payload.get("data", {})
    status = (data.get("status") or "").lower()

    if status not in ("successful", "success"):
        GATE("WEBHOOK:Ignored non-success payment", extra=status)
        return "Ignored", 200

    GATE("WEBHOOK:Payment marked successful")

    order_id = data.get("tx_ref")
    paid_amount = int(float(data.get("amount", 0)))
    currency = data.get("currency")

    GATE("WEBHOOK:Payment details",
         extra=f"order={order_id} amount={paid_amount} currency={currency}")
    ...
    if not row:
        GATE("WEBHOOK:FAIL order not found", extra=order_id)
        return "Order not found", 200

    user_id, movie_id, expected_amount, paid = row
    GATE("WEBHOOK:Order loaded",
         extra=f"user={user_id} expected={expected_amount} paid={paid}")

    if paid == 1:
        GATE("WEBHOOK:Already processed", extra=order_id, uid=user_id)
        return "Already processed", 200

    if paid_amount != expected_amount or currency != "NGN":
        GATE("WEBHOOK:FAIL wrong amount/currency",
             extra=f"got={paid_amount} expected={expected_amount} {currency}",
             uid=user_id)

        bot.send_message(
            user_id,
            "❌ An samu matsala a biyan kuɗi. Tuntuɓi admin."
        )
    ...
    GATE("WEBHOOK:Order marked PAID", uid=user_id)
    ...
    GATE("WEBHOOK:User notified", uid=user_id)
...
"""
        ...
        GATE("WEBHOOK:Admin group notified", extra=str(PAYMENT_NOTIFY_GROUP))

    GATE("WEBHOOK:END success flow", uid=user_id)
    return "OK", 200
    
def send_feedback_prompt(user_id, tx_ref):
    exists = conn.execute(
    ...
    bot.send_message(
        user_id,
        "Ina fatan ka ji daɗin siyayya 🥰\nDan Allah ka zaɓi yadda kake ji 👇",
        reply_markup=kb
    )
...
def send_allfilms_page(uid, page_index):  
  ...
  # ===== BUTTONS =====    
    ...
    kb.add(  
        ...
        InlineKeyboardButton("📺 Our Channel", url="https://t.me/YOUR_CHANNEL")  
    )  
...
def deliver_movies(call):
    ...
    # ---------- Tabbatar da biyan ----------
    row = cur.execute(
        "SELECT movie_id, paid FROM orders WHERE id=? AND user_id=?",
        (order_id, user_id)
    ).fetchone()
    ...
    if already:
        bot.answer_callback_query(call.id, "ℹ️ Ka riga ka karɓi fina-finanka")
        return

    bot.answer_callback_query(call.id, "📤 Ana turo maka fina-finai...")

    # ---------- Nemo duk movies na series ----------
    movies = cur.execute(
        "SELECT id, file_id, title FROM movies WHERE series_id=?",
        (series_id,)
    ).fetchall()

    if not movies:
        bot.send_message(user_id, "❌ Babu fim a wannan series")
        return

    sent = 0

    for movie_id, file_id, title in movies:
        try:
            bot.send_video(
                user_id,
                file_id,
            ...
            print("Send movie error:", e)

    conn.commit()

    bot.send_message(
        user_id,
        f"✅ An tura duk fina-finanka ({sent}).\n"
        "Ba za a sake tura su ba."
    )
...
def build_cart_view(uid):
    ...
    for mid, title, price in rows:
        ...
        kb.add(
            InlineKeyboardButton(
                f"❌ Cire {title}",
                callback_data=f"removecart:{mid}"
            )
        )
...
def sendmovie_cmd(m):
    ...
    if len(parts) < 2:
        bot.send_message(uid, "Amfani: /sendmovie <movie_id>")
        return

    try:
        movie_id = int(parts[1])
    except:
        bot.send_message(uid, "Movie ID bai dace ba.")
        return
    ...
    if not row:
        bot.send_message(uid, "❌ Ba a samu wannan fim ba.")
        return
...
def build_unpaid_orders_view(uid, page):
    ...
    for oid, amount, title in rows:
        name = title if title else "Deleted movie"
...
def build_paid_orders_view(uid, page):
    ...
    InlineKeyboardButton(
        ...
        kb.add(InlineKeyboardButton("⤴️ KOMA FARKO", callback_data="go_home"))
        return "📦 <b>Babu paid orders.</b>", kb
...
def series_finalize(m):
    ...
    kb.add(
        ...
        InlineKeyboardButton(
            "💳 Buy now",
            callback_data=f"groupitem:{series_id}"
        )
    )
...
def handle_callback(c):
    ...
    # ===== GATE: CALLBACK RECEIVED =====
    GATE("CALLBACK:RECEIVED", extra=data, uid=uid)
    ...
    bot.answer_callback_query(c.id)
    # ================= BUY SERIES =================
    if data.startswith("groupitem:"):
        try:
            series_id = int(data.split("groupitem:", 1)[1])
        except:
        ...
        if not row:
            bot.answer_callback_query(c.id, "❌ Series not found")
            return

        title = row["title"]
        price = int(row["price"] or 0)

        # 🔢 adadin movies a series
        movie_count = conn.execute(
            "SELECT COUNT(*) FROM movies WHERE series_id=?",
            (series_id,)
        ).fetchone()[0]

        if movie_count <= 0:
            bot.answer_callback_query(c.id, "❌ Babu movie a wannan series")
            return

        # 🔒 1️⃣ DUBA KO AKWAI TSOHON UNPAID ORDER NA WANNAN SERIES
        old = conn.execute(
        ...
        if old:
            ...
            bot.send_message(
                uid,
                f"""⚠️ <b>Kana da series order da bai kammala ba</b>
...
⚠️ <i>Idan ka riga ka biya amma baka samu series ba,
tura wannan Order ID ga admin.</i>
...
👇 Ci gaba da biyan wannan order:""",
        ...
        # 2️⃣ IDAN BABU → KIRKIRI SABON ORDER
        order_id = str(uuid.uuid4())
        conn.execute(
            "INSERT INTO orders (id, user_id, movie_id, amount, paid) VALUES (?, ?, ?, ?, 0)",
...
⚠️ <i>Ajiye wannan Order ID sosai.
Idan ka biya amma baka samu series ba,
admin zai yi amfani da shi.</i>
...
👇 Danna ƙasa domin biya:""",
    ...
    # ================= BUY NOW (DM) =================
    if data.startswith("buydm:"):
        try:
            mid = int(data.split("buydm:", 1)[1])
        except:
        ...
        # 🔍 DUBA KO AKWAI TSOHON UNPAID ORDER
        old = conn.execute(
            "SELECT id FROM orders WHERE user_id=? AND movie_id=? AND paid=0",
            (uid, movie_id)
        ).fetchone()
...
⚠️ <i>Ajiye wannan Order ID.
Idan ka biya baka samu fim ba, admin zai nema.</i>
""",
            parse_mode="HTML",
            reply_markup=kb
        )
...
⚠️ <i>Ajiye wannan Order ID.
Idan matsala ta taso bayan biya, admin zai nema.</i>
"""
        ...
        if chat_type != "private":
            bot.answer_callback_query(c.id, "An tura zuwa DM.")
    ...
    if data == "my_movies":
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("🧲 1 Year ago", callback_data="resend:365"))
        kb.add(InlineKeyboardButton("🧲 4 Months ago", callback_data="resend:120"))
        kb.add(InlineKeyboardButton("🔍 2 Months ago", callback_data="resend:60"))
        kb.add(InlineKeyboardButton("🔍 2 Weeks ago", callback_data="resend:14"))

        bot.send_message(
            uid,
            "🤫 Anan zaka iya sake karɓar fina-finan da **ka taba siya** kawai 🎬",
            reply_markup=kb
        )
        bot.answer_callback_query(c.id)
        return
    ...
    if data.startswith("resend:"):
        ...
        if not rows:
            bot.send_message(uid, "❌ Babu fim da ka siya a wannan lokacin.")
            bot.answer_callback_query(c.id)
            return
        ...
        if sent == 0:
            bot.send_message(uid, "⚠️ Ba a samu fim da za a tura ba.")
        else:
            bot.send_message(uid, f"✅ An sake tura fina-finai {sent}.")
...
def buy_weekly_handler(c):
    ...
    if not movie_ids:
        bot.answer_callback_query(c.id, "Weekly list empty.")
        return
...
def _create_and_send_buyall(uid, items, c):
    ...
    # 🔍 DUBA TSOHON UNPAID BUY-ALL ORDER
    old = conn.execute(
...
⚠️ <i>Ajiye wannan Order ID.
Idan ka biya amma baka samu movies ba,
admin zai yi amfani da shi.</i>
"""
    ...
    try:
        ...
        bot.answer_callback_query(c.id, "Open bot DM first.")
...
def handle_cancel_order_cb(c):
    ...
    conn.execute("DELETE FROM orders WHERE id=? AND user_id=? AND paid=0", (order_id, uid))
    conn.commit()
